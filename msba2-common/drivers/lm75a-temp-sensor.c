/*
 * lm75a-temp-sensor.c - Driver for the LM75A temperature sensor using the LPC2387 and based on the i2c interface.
...
volatile bool my_alarm = false; //wichtig sonst wird kein Interrupt gelˆst.
...
static void to_bytes(float_t f, uint8_t* buff) {
	...
	buff[0] = (uint8_t)((i >> LM75A_BIT1) & 0xFF); //Most significant byte
	buff[1] = (uint8_t)((i & LM75A_BIT1) << LM75A_BIT7); //Least significant byte
}
...
static uint16_t lm75A_get_register_value(uint8_t i2c_interface,
	...
	if ((reg_size > 0) && (reg_size < 3)) {
		...
		if (!status) { //Slave is not ready
			puts(
					"[lm75a_tempSensorI2C/lm75A_getConfigReg]: Slave is not ready !\n");
			if (reg_size < 2) {
				return UCHAR_MAX;
			} else {
				return UINT16_MAX;
