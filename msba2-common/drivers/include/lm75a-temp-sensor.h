 ...
 *
 *@param[in] external_interr_handler	pointer to an external task handler which is performed, if an external interrupt is occurred and the external subroutine is leaved.
 *										This parameter is optional, the NULL-value can be entered.
 */
 ...
 * 			 The baud rate and the handler for the external interrupt can be initialized. The external interrupt is optional,
 * 			 if no handler is available, the NULL-value can be entered. The hysteresis and the over-temperature are displayed before and after a rest. After this the LM7A sensor
 * 			 is set in the interrupt or the comparator mode.
 *
