/*
 * i2c.h - Definitions for the I2C interface and the functions for the LPC2387 chip.
 ...
 * 		The user can optionally give its own master interrupt handler. If the user does not declare a handler,
...
/**
 * @brief	Initialize the I2C-Interface with the appropriate operation mode, baud rate and other parameter.
 ...
 * 			The read values are stored in a buffer, which it is handed to the function.
 *
 *
 * @param[in] i2c_interface		the i2c interface, several interfaces can be selected: i2c0, i2c1 and i2c2.
 * @param[in] slave_addr		slave address
 * @param[in] reg_addr			a register address, which is residing in the slave device.
 * @param[out] rx_buff			a pointer to a receive buffer.
 * @param[in] rx_buff_length	the number of receiving bytes resp. the receive-buffer length.
 *
 * @return true if the read transaction is successfully completed, otherwise false.
 */
 ...
 * 			The values are stored in a buffer, which it is handed to the function.
 *
 *
 * @param[in] i2c_interface		the i2c interface, several interfaces can be selected: i2c0, i2c1 and i2c2.
 * @param[in] slave_addr		slave address
 ...
 * @param[in] tx_buff_length	the number of transmitting bytes resp. the transmit-buffer length.
 *
 * @return true if the write transaction is successfully completed, otherwise false.
 */
 ...
 * 			The sending/receiving bytes are stored in two buffers, which are handed to the function.
 *
 * @param[in] i2c_interface		the i2c interface, several interfaces can be selected: i2c0, i2c1 and i2c2.
 * @param[in] slave_addr		slave address
 * @param[in] tx_buff			pointer to a sending buffer.
 * @param[in] tx_buff_length	the number of sending bytes resp. the send-buffer length.
 * @param[out] rx_buff			pointer to a receive buffer.
 * @param[in] rx_buff_length	the number of receiving bytes resp. the receive-buffer length.
 *
 * @return true if the write/receive transaction are successfully completed, otherwise false.
 */
bool i2c_trans_receive(uint8_t i2c_interface, uint8_t slave_addr,
		uint8_t* tx_buff, uint8_t tx_buff_length, uint8_t* rx_buff,
		uint8_t rx_buff_length);
 ...
 * 			 It selects the respective Serial Data Line (SDAx) and Serial Clock (SCLx).
 *
 * @param[in] i2c_interface		the i2c interface, several interfaces can be selected: i2c0, i2c1 and i2c2.
 *
 */
 ...
 *
 * @return true if the the interrupt handler is successfully registered, otherwise false.
 *
 */
...
/**
 * @brief	This routine complete a I2C transaction from start to stop conditions.
 * 			All the intermitten steps are handled in the interrupt handler.
 *
 * @param[in] i2c_interface		the i2c interface, several interfaces can be selected: i2c0, i2c1 and i2c2.
 *
 * @return false if the start condition can never be generated and timed out, otherwise true.
 *
 */
...
/**
 * @brief	 Enable master transmitter mode for the appropriate i2c-interface.
 *
 * @param[in] i2c_interface		the i2c interface, several interfaces can be selected: i2c0, i2c1 and i2c2.
 *
 */
...
/**
 * @brief	Enable the on-chip pull-up resistor for the appropriate i2c-interface.
 *
 * @param[in] i2c_interface		the i2c interface, only the i2c1 and the i2c2-interfaces are supported.
 */
void i2c_enable_pull_up_resistor(uint8_t i2c_interface);


/**
 * @brief	Disable the on-chip pull-up resistor for the appropriate i2c-interface.
 *
 * @param[in] i2c_interface		the i2c interface, only the i2c1 and the i2c2-interfaces are supported.
 */
