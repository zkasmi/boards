...
static void i2c_interface0_master_handler(void) __attribute__((interrupt("IRQ")));
static void i2c_interface1_master_handler(void) __attribute__((interrupt("IRQ")));
static void i2c_Interface2_master_handler(void) __attribute__((interrupt("IRQ")));
...
void i2c_clear_control_register(uint8_t i2c_interface) {
	...
	case I2C0:
		I20CONCLR =
				I2CONCLR_AAC | I2CONCLR_SIC | I2CONCLR_STAC | I2CONCLR_I2ENC;
		break;
	case I2C1_0:
	case I2C1_1:
		I21CONCLR =
	...
	case I2C0:
		if (handler == NULL) {
			successful = install_irq(I2C0_INT,
					(void *) i2c_interface0_master_handler, HIGHEST_PRIORITY);
		} else {
	...
	case I2C1_1:
		if (handler == NULL) {
			successful = install_irq(I2C1_INT,
					(void *) i2c_interface1_master_handler, HIGHEST_PRIORITY);
		} else {
	...
	case I2C2:
		if (handler == NULL) {
			successful = install_irq(I2C2_INT,
					(void *) i2c_Interface2_master_handler, HIGHEST_PRIORITY);
		} else {
...
bool i2c_trans_receive(uint8_t i2c_interface, uint8_t slave_addr,
	...
	if (tx_buff != NULL && (tx_buff_length > 0)) {
		...
		puts(
				"[i2c.c/i2cRead]: the txBuff is not valid or has not a valid length value !\n");
		return false;
	}
...
//	}
	...
	case 0x30:
		...
		if (wr_index != i2c_write_length) {
			I20DAT = i2c_master_buffer[1 + wr_index]; /* this should be the last one */
			if (wr_index != i2c_write_length) {
				i2c_master_state = DATA_ACK;
			} else {
...
//	}
	...
	case 0x30:
		//puts("Data byte has been transmitted\n");
		if (wr_index != i2c_write_length) {
			I21DAT = i2c_master_buffer[1 + wr_index]; /* this should be the last one */
			if (wr_index != i2c_write_length) {
				i2c_master_state = DATA_ACK;
			} else {
				i2c_master_state = DATA_NACK;
...
//	}
	...
	case 0x30:
		//puts("Data byte has been transmitted\n");
		if (wr_index != i2c_write_length) {
			I22DAT = i2c_master_buffer[1 + wr_index]; /* this should be the last one */
			if (wr_index != i2c_write_length) {
				i2c_master_state = DATA_ACK;
			} else {
				i2c_master_state = DATA_NACK;
...
void i2c_enable_pull_up_resistor(uint8_t i2c_interface) {
	...
	case I2C1_0: // P0.0  SDA1, P0.1  SCL1
		puts("The on-chip pull-up resistor is for the I2C1_0 enabled");
		PINMODE0 &= ~(BIT0 | BIT1 | BIT2 | BIT3);
		break;

	case I2C1_1: // P0.19 SDA1, P0.20 SCL1
		puts("The on-chip pull-up resistor is for the I2C1_1 enabled");
		PINMODE1 &= ~(BIT6 | BIT7 | BIT8 | BIT9);
		break;

	case I2C2: // P0.10 SDA2, P0.11 SCL2
		puts("The on-chip pull-up resistor is for the I2C2 enabled");
...
void i2c_disable_pull_up_resistor(uint8_t i2c_interface) {
	switch (i2c_interface) {
	case I2C1_0: // P0.0  SDA1, P0.1  SCL1
		puts("The on-chip pull-up resistor is for the I2C1_0 disabled");
		PINMODE0 &= ~(BIT0 | BIT2);
		PINMODE0 |= (BIT1 | BIT3);
		break;

	case I2C1_1: // P0.19 SDA1, P0.20 SCL1
		puts("The on-chip pull-up resistor is for the I2C1_1 disabled");
		PINMODE1 &= ~(BIT6 | BIT8);
		PINMODE1 |= (BIT7 | BIT9);
		break;

	case I2C2: // P0.10 SDA2, P0.11 SCL2
		puts("The on-chip pull-up resistor is for the I2C2 disabled");
		PINMODE0 &= ~(BIT20 | BIT22);
